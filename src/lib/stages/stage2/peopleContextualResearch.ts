...
function primaryUsersHeuristic(
  ...
  const profiles: Record<ProblemDomain, string[]> = {
    cafe: [
      "· 직장인·오피스 근무자 (25~40대, 평일 통근·점심 시간 카페 이용): 오피스 밀집 상권에서 테이크아웃·매장 이용이 잦은 핵심 소비 세그먼트입니다.",
      "· 대학생·취준생 (20대 초중반, 대학가·스터디 카페 체류): 가격·와이파이·좌석을 중시하며 한가·혼잡 시간대 이용 패턴이 갈리는 세그먼트입니다.",
      "· 주부·부모 (30~50대, 동네·마트 인근 카페 방문): 아이 동반·심부름 겸 방문하는 생활 밀착형 이용자 세그먼트입니다.",
    ],
    app: [
      "· 일상 사용자 (20~40대, 해당 서비스를 주 1회 이상 쓰는 개인): 앱·서비스를 직접 실행하는 최종 이용자 세그먼트입니다.",
      "· 초보·고령 사용자 (50대 이상 또는 디지털 숙련도 낮음): 온보딩·인증·용어에 익숙하지 않은 이용자 세그먼트입니다.",
      "· 파워 사용자 (고빈도·숙련 이용자): 단축·자동화·정보 밀도를 중시하는 헤비 유저 세그먼트입니다.",
    ],
    public: [
      "· 서비스 이용자·환자 (해당 공공·의료·교육 서비스를 직접 받는 사람): 접수·대기·안내를 겪는 핵심 이용자 세그먼트입니다.",
      "· 학부모·보호자 (자녀·피보호자를 대신해 의사결정·동행): 본인이 아닌 대상을 위해 서비스를 이용하는 세그먼트입니다.",
      "· 신규·재방문 이용자 (처음 방문 또는 간헐적 이용): 절차·안내 이해 수준이 달라 분리해 볼 가치가 있는 세그먼트입니다.",
    ],
    generic: [
      "· 핵심 이용자 (문제를 가장 직접·반복적으로 겪는 사람): 1단계 문제점을 일상 맥락에서 체감하는 주 사용자 후보입니다.",
      "· 간접 영향 받는 사람 (가족·동료·이웃): 본인이 아니어도 문제의 파급을 받을 수 있는 인접 세그먼트입니다.",
      "· 초보·숙련 이용자 (경험 수준별): 같은 문제라도 사용 경험에 따라 행태·기대가 달라지는 세그먼트입니다.",
    ],
  };

  const traitsBehaviors: Record<ProblemDomain, string[]> = {
    cafe: [
      "· 직장인: 주 3~5회 음료 구매, 출근 전·점심 피크에 집중 방문, 앱·현장 주문·픽업 동선을 루틴화하는 패턴입니다.",
      "· 대학생·취준생: 장시간 체류·와이파이·가격 민감, 혼잡 시간대에는 좌석·대기 회피 또는 이탈하는 행태입니다.",
      "· 주부·부모: 동반 방문·메뉴 선택 부담, 매장 동선·대기 공간·안전을 우선 고려하는 이용 패턴입니다.",
    ],
    app: [
      "· 일상 사용자: 검색·비교·결제를 한 흐름으로 끝내려 하며, 단계가 끊기면 재시도·대안 탐색을 반복하는 행태입니다.",
      "· 초보·고령 사용자: 도움말·피드백 없이는 진입을 미루거나 중단하는 패턴이며, 익숙한 대체 수단을 유지하는 경향입니다.",
      "· 파워 사용자: 단축·자동완성·정보 밀도를 기대하고, 불편 누적 시 빠르게 대체 서비스로 이동하는 행태입니다.",
    ],
    public: [
      "· 이용자·환자: 절차·서류·대기 시간에 민감하며, 안내 부재 시 창구·콜센터에 재문의하는 패턴입니다.",
      "· 학부모·보호자: 대리 신청·동행·서류 준비를 맡으며, 정보 비대칭 속에서 추가 확인을 반복하는 행태입니다.",
      "· 신규·재방문 이용자: 안내·표지판·직원 응대에 의존하며, 첫 방문과 재방문의 기대·불안이 다른 패턴입니다.",
    ],
    generic: [
      "· 핵심 이용자: 문제가 반복되는 시간대·상황·목적이 뚜렷하며, 해결 시도·대안 탐색을 스스로 기록할 수 있는 행태입니다.",
      "· 간접 영향 집단: 본인 일정·비용·스트레스로 문제 파급을 체감하나, 직접 해결 주체는 아닌 패턴입니다.",
      "· 숙련도별 이용자: 같은 기능에도 학습 곡선·기대치가 달라, 인터뷰·관찰 질문을 나눠야 하는 행태입니다.",
    ],
  };

  const toKnowBridge: Record<ProblemDomain, string[]> = {
    cafe: [
      `· 직장인 세그먼트: 「${clipProblem(problem, 48)}」이 출근 전·점심 피크에서 어떻게 드러나는지 주문~수령 흐름을 현장에서 확인합니다.`,
      "· 세그먼트별로 이용 빈도·선호 채널(앱·키오스크·카운터)을 To-know 질문으로 구체화해 현장 조사에 연결.",
    ],
    app: [
      `· 각 세그먼트별로 「${clipProblem(problem, 48)}」이 발생하는 화면·단계·상황을 Diary·인터뷰로 검증합니다.`,
      "· 초보 vs 파워 사용자는 같은 기능에서도 실패 지점이 달라, To-know를 나눠 작성하는 것이 좋습니다.",
    ],
    public: [
      `· 이용자·보호자 세그먼트별로 「${clipProblem(problem, 48)}」이 접수·대기·안내 중 어디서 막히는지 현장에서 확인합니다.`,
      "· 절차 이해 수준·방문 빈도에 따라 To-know 질문을 분리해 실제 리서치 대상을 좁힙니다.",
    ],
    generic: [
      `· 위 세그먼트마다 「${clipProblem(problem, 48)}」을 누가·언제·어떤 목적으로 겪는지 To-know 질문으로 구체화합니다.`,
...
function stakeholdersHeuristic(
  ...
  const profiles: Record<ProblemDomain, string[]> = {
    cafe: [
      "· 카페 사장·운영자 (매장 손익·운영 정책 결정): 메뉴·인력·동선·프로모션을 정하는 경영 주체입니다. 대기·주문 방식이 매출·회전율·인건비와 맞물립니다.",
      "· 바리스타·홀·아르바이트 (현장 제조·수납·안내): 손님과 맞닿아 문제를 가장 먼저 흡수하는 실행 역할입니다. 피크 타임·클레임·재고 이슈가 여기서 드러납니다.",
      "· 원두·장비·소모품 공급사 (B2B 파트너): 매장 운영 조건·키오스크·POS 연동에 영향을 주는 공급망 이해관계자입니다.",
      "· 배달·예약 플랫폼 담당 (중개 채널): 주문이 앱으로 넘어가면 대기·픽업 흐름이 플랫폼 정책과 맞물립니다.",
    ],
    app: [
      "· 서비스 기획·PM (기능·정책 결정): 사용자 흐름·알림·온보딩을 설계하는 제품 책임 역할입니다.",
      "· 개발·QA·운영 (구현·배포·장애 대응): 기술 제약·버그·성능이 체감 문제로 전달되는 지점을 만듭니다.",
      "· 마케팅·CS (유입·이탈·VoC 수집): 사용자 불만·요청이 정형화되어 들어오는 창구입니다.",
      "· 제휴·플랫폼 파트너: 외부 연동·정산·정책이 서비스 경험에 간접 영향을 줍니다.",
    ],
    public: [
      "· 행정·기관 담당자 (절차·규정 운영): 민원·접수·승인 흐름을 관리하는 정책 실행 주체입니다.",
      "· 현장 실무·강사·의료진 (직접 서비스 제공): 이용자와 맞닿아 규정과 현실의 간극을 체감합니다.",
      "· 감독·평가·예산 부서: 성과·컴플라이언스 기준이 현장 선택에 제약을 겁니다.",
    ],
    generic: [
      "· 운영·관리 담당 (내부 의사결정): 문제 해결 방향·우선순위·예산을 조율하는 조직 내 이해관계자입니다.",
      "· 파트너·공급사 (외부 협력): 제품·서비스 품질·일정·계약 조건으로 문제의 원인·해결 수단에 영향을 줍니다.",
      "· 규제·심사·감독 기관 (간접): 허용 범위·보고 의무가 솔루션 선택을 좁힐 수 있습니다.",
  ...
  const roles: Record<ProblemDomain, string[]> = {
    ...
    app: [
      "· 기획·개발은 제품 설계·기술 부채를 통해 문제를 재현·완화·악화시킬 수 있습니다.",
...
- 섹션 1 「누구인지 (주 사용자 프로필)」: 3~4개 세그먼트. 각 항목 형식 「세그먼트명 (인구·역할·한 줄 정체성)」: **누구인지**를 한 문장으로 (존댓말 ~습니다/~입니다).
- 섹션 2 「특성·행태」: 각 세그먼트의 라이프스타일·이용 빈도·선호·의사결정·행동 패턴 3~4개 불릿. 형식 「세그먼트: 특성·행태 설명」. 불편·니즈 나열이 아닌 **프로필·행태** 중심. 모두 존댓말.
- 섹션 3 「To-know·현장 조사 연결 (가설)」: 위 프로필을 바탕으로 To-know list·현장 리서치에서 확인할 방향 2~3개 불릿. 존댓말.
- 600~900자. 질문형 금지. ~이다/~다/~함 등 평서체 금지.
- ${COACH_EMPATHY_MAP_PROMPT_RULE}
- Diary Studies·프로필 예시를 줄 때도 Pain/Gain·고민·희망 칸을 만들지 마세요.`.trim();
  }
...
- 섹션 1 「누구인지 (이해 관계자 프로필)」: 3~4명. 각 항목 「역할·조직 라벨 (한 줄 정체성)」: **누구·어떤 조직**인지 1문장 + **무슨 일을 하는지** 1~2문장 (존댓말 ~습니다/~입니다).
- 섹션 2 「역할·영향」: 문제·해결에 미치는 **의사결정·실행·중개** 관계 2~3개 불릿. 존댓말.
- 섹션 3 「문제와의 연결 (가설)」: 주 사용자와 다른 **간접** 연결 1~2개 불릿. 존댓말.
- 600~900자. 질문형 금지. ~이다/~다/~함 등 평서체 금지.
- ${COACH_EMPATHY_MAP_PROMPT_RULE}`.trim();
}

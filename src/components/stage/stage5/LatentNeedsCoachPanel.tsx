...
- 핵심 니즈 선별 탭에서는 문제·Pain·반복·HMW 전환 용이성을 기준으로 핵심 니즈가 미리 골라져 있으며, 사용자는 사분면에서 확인하고 조정합니다. 보류는 폐기가 아니라 잠시 접어두는 것입니다.`;
...
function buildCoreSelectionMessages(
  ...
  const messages: CoachDialogItem[] = [
    ...
    },
    {
      type: "bubble",
      content: formatCoachDialogBreaks(
        "사분면에서 확인해 보세요. 세로는 중요도(고통이 크고 자주 겪는가), 가로는 해결 공백(쓸 만한 대안이 없는가)이에요. 오른쪽 위에 모인 카드가 핵심 후보예요.",
      ),
    },
    {
      type: "bubble",
      content: formatCoachDialogBreaks(
        "근거 배지나 「왜 골랐나요」를 손보면, 나중에 되돌아볼 때 도움이 돼요. 핵심이 아니라고 본 카드는 보류함으로 — 버리는 게 아니라 잠시 접어두는 거예요.",
      ),
    },
  ];
  ...
  if (data.coreNeedIds.length > 0) {
    messages.push({
      type: "bubble",
      content: formatCoachDialogBreaks(
        `지금 핵심 니즈 ${data.coreNeedIds.length}개가 골라져 있어요. 맞는지 한 번 훑어보고, 흔들리면 보류함과 바꿔도 좋아요. 선별도 가설이에요.`,
      ),
    });
  }

...
export function HmwCoachPanel({ projectId, data, variant }: HmwCoachPanelProps) {
    ...
    return [
      ...
      },
      {
        ...
        content: formatCoachDialogBreaks(
          "6단계에서 정리한 잠재 니즈를 HMW 질문으로 바꿔 보세요. Kevin이 좋은 점 증폭·불편 해소·반대 탐색 세 갈래로 초안을 만든 뒤 가장 알맞은 하나를 채워 두었어요. 아직 답이 아니라 씨앗이에요 — 판단은 잠시 미루고, 와일드한 수정도 환영해요.",
        ),
      },
    ];
  }, [purpose]);
  ...
  const chatContext = useMemo(
    () => ({
      ...
      stageBehaviorNote:
        "7단계 HMW: 잠재 니즈마다 Amp up / Remove bad / Explore opposite 세 변주 중 하나를 고릅니다. 형식은 「어떻게 하면 ~하기 위해 ~할 수 있을까?」. 목적·제약(왜)과 구체 상태(무엇을)를 담아 다음 아이디어 단계 시드로 씁니다. 판단 유예·와일드 아이디어를 독려합니다.",
    }),
    [projectId, data],
  );
